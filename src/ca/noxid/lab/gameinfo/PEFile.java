 ...
 * The hope is that having something this generic will reduce the chance of any faults going unseen.
 * THE FULL LIST OF ASSUMPTIONS:
 * 1. Tools expect (but we don't) sections to be in virtual order (this is my fault -- PEONS dev)
 * 2. Tools and us expect .text to be at FA 0x1000, and for .text/.rdata/.data to be linearized.
 *    If we can't ensure this, we fail for safety. (The value is adjustible per-game, but cannot be eliminated)
 * 3. Old-Style Relocations and Line Numbers never happen. If we encounter them, we fail for safety.
 * 4. The executable is assumed to not have relocation data. If it does, we ignore it - it's assumed we already know and can handle it.
 * 5. The resource chunk starts at its section and continues to the end of the chunk.
 * If this is not the case, then Xin probably started cannibalizing it and we should just pretend it's not a resource section,
...
public class PEFile {
    ...
    public PEFile(ByteBuffer source, int expectedTex) throws IOException {
        ...
        earlyHeaderBB.getShort(); // We don't actually need to check the machine - if anything is wrong, see opt.header magic
    ...
    private int test(boolean justOrderAndOverlap) throws IOException {
            ...
            for (Section s : sections) {
                ...
                if (s.metaLinearize) {
                    if (alignForward(s.virtualAddrRelative, fileAlignment) != s.virtualAddrRelative)
                        System.err.println("Warning: File alignment being broken for linearization. This isn't a critical error, but it's not a good thing.");
        ...
        setOptionalHeaderInt(0x38, imageSize);
    ...
    // This is also used in cases where we need to know the current end of virtual memory.
    ...
    // Add in the given section and ensure the resources section is the latest section.
    ...
    private void mallocInterior(Section resourcesSection, int i, int sectionAlignment) {
        while (true) {
            ...
            boolean hit = false;
            for (Section s : sections) {
                if (new AllocationSpan(s.virtualAddrRelative, alignForward(s.virtualSize, sectionAlignment)).collides(as)) {
                    hit = true;
                    break;
                }
            }
     ...
     *  but it's a 2MB EXE at max. and frankly this code has to be as readable as possible.
     */
    public static class Section {
        ...
        // File address is implied by linearization or automatic rearranging during write()
        ...
        public String toString() {
            return decodeTag() + " : RVA " + Integer.toHexString(virtualAddrRelative) + " VS " + Integer.toHexString(virtualSize) + " : RDS " + Integer.toHexString(rawData.length) + " : CH " + Integer.toHexString(characteristics);
        }

        // -- Copied from the old ExeSec, these routines appear reliable so let's not break anything --
    ...
    private class AllocationSpan {
        ...
        public boolean collides(AllocationSpan as) {
            return within(as.start) || within(as.start + as.length - 1) || as.within(start) || as.within(start + length - 1);
        }
